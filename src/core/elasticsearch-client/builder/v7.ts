...
import { compareVersion, parseJsonWithBigIntSupport, stringifyJsonWithBigIntSupport } from "$/util";
...
export class ElasticsearchClientV7 extends ElasticsearchClientCommon {
  ...
  async templateList(): Promise<Array<IndexTemplateListItem>> {
    ...
    // 如果版本大于等于7.8
    if (compareVersion(this.props.version, "7.8") > 0) {
      const indexTemplateStr = await this.request({
        method: "GET",
        url: "/_index_template"
      });
      parseJsonWithBigIntSupport<{index_templates: Array<any>}>(indexTemplateStr)
