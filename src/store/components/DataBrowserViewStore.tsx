...
import { DataBrowserView } from "@/entity";
import { addDataBrowserView, deleteDataBrowserView, listDataBrowserViews } from "@/api";
...
export const useDataBrowserViewStore = defineStore("dataBrowserView", () => {
    ...
    const dialog = DialogPlugin({
      default: () => (
        <div>
          ...
          >
            <Paragraph>
              在本客户端中，“视图”是一种通过 索引名称的通配符模式（如 <Text code>logs-*</Text>
              ）来动态匹配多个 Elasticsearch 索引的方式。
            </Paragraph>
            <Paragraph>
              <ul>
                <li>
                  例如：输入 <Text code>logs-*</Text> 将自动包含所有以 <Text code>logs-</Text>{" "}
                  开头的索引（如 <Text code>logs-2025-12-01</Text>、<Text code>logs-app-prod</Text>{" "}
                  等）。
                </li>
                <li>
                  支持的通配符：
                  <ul>
                    <li>
                      <Text code>*</Text>：匹配任意字符（包括空字符），例如{" "}
                      <Text code>app-*-prod</Text>
                    </li>
                    <li>
                      <Text code>?</Text>：匹配单个任意字符，例如 <Text code>log-2025-12-??</Text>
                    </li>
                  </ul>
                </li>
  ...
  const remove = async (id: string, label: string) => {
    MessageBoxUtil.alert(`是否删除视图「${label}」`, "确认删除").then(() => {
      deleteDataBrowserView(urlId.value!, id)
        .then(() => {
