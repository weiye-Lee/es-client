...
export function execAdd(indexName: string, initData: string): Promise<string> {
  ...
  return new Promise(resolve => {
    let modalReturn = Modal.open({
      ...
      width: "80vw",
      draggable: true,
      content: () => <MonacoEditor height={`calc(90vh - 162px)`} language="json" v-model={data.value}/>,
      footer: () => <>
        <Button type="text"
...
export function execUpdate(indexName: string, id: string, initData: string): Promise<{
  ...
  const data = ref(formatJsonString(initData));
  return new Promise(resolve => {
    let modalReturn = Modal.open({
      ...
      footer: () => <>
        ...
        <Button onClick={() => modalReturn.close()}>取消</Button>
        <Button type="primary" onClick={() => {
          resolve({id, data: data.value, close: modalReturn.close});
        }}>修改</Button>
      </>
    });
  });
